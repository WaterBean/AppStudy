...
final class LoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        NotificationCenter.default.addObserver(self, selector: #selector(handleAuthenticationSuccessful(_:)), name: Notification.Name("AuthenticationSuccessful"), object: nil)

        configure()
        setupAutoLayout()
    }
    @objc func handleAuthenticationSuccessful(_ notification: Notification) {
        guard let isAuthenticationSuccessful = notification.userInfo?["isAuthenticationSuccessful"] as? Bool, isAuthenticationSuccessful else {
            return
        }
        
        // 정상 인증 상태이므로 successViewController로 이동
    ...
    private func configure() {
        ...
        [stackView,warningLabel].forEach { view.addSubview($0) }
    ...
    private func setupAutoLayout() {
        ...
        warningLabel.topAnchor.constraint(equalTo: stackView.bottomAnchor, constant: -textViewHeight-44).isActive = true
        warningLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 30).isActive = true
        warningLabel.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -30).isActive = true
        warningLabel.heightAnchor.constraint(equalToConstant: textViewHeight).isActive = true
    ...
    @objc func loginButtonTapped() {
        ...
        if emailTextField.text! != userID {
            warningLabel.text = "존재하지 않는 ID입니다."
        } else if passwordTextField.text! != userPWD {
            warningLabel.text = "패스워드가 일치하지 않습니다."
        } else {
            
            let spVC = SecondaryPasswordViewController()
            present(spVC, animated: true, completion: nil)
            
            //        let scVC = SuccessViewController()
